...
class RulePackageModel(Base):
    """A versioned bundle of rules for cross-organization sharing."""
    ...
    publisher_id: Mapped[str] = mapped_column(String(255), nullable=False, default="system")
    description: Mapped[str] = mapped_column(Text, nullable=False, default="")
    ...
    __table_args__ = (sa.UniqueConstraint("name", "version", "publisher_id", name="uq_package_name_version_publisher"),)

    rules: Mapped[list["PackageRuleModel"]] = relationship(back_populates="package")


class PackageRuleModel(Base):
    """Associates a rule with a package, with a stable in-package ID."""

    __tablename__ = "package_rules"

    id: Mapped[str] = mapped_column(Uuid, primary_key=True, default=uuid4)
    package_id: Mapped[str] = mapped_column(
        Uuid, ForeignKey("rule_packages.id", ondelete="CASCADE"), nullable=False, index=True
    )
    rule_id: Mapped[str] = mapped_column(Uuid, ForeignKey("rules.id", ondelete="CASCADE"), nullable=False, index=True)
    package_rule_id: Mapped[str] = mapped_column(String(100), nullable=False)
    test_cases: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)

    package: Mapped["RulePackageModel"] = relationship(back_populates="rules")


class PackageSubscriptionModel(Base):
    """A project's subscription to a rule package."""
    ...
    version_constraint: Mapped[str] = mapped_column(String(50), nullable=False, default="*")
    auto_update: Mapped[bool] = mapped_column(default=False)
    composition_policy: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
...
class CompositionConflictModel(Base):
    """A detected conflict between rules from different packages."""

    __tablename__ = "composition_conflicts"

    id: Mapped[str] = mapped_column(Uuid, primary_key=True, default=uuid4)
    project_id: Mapped[str] = mapped_column(
        Uuid, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True
    )
    rule_a_id: Mapped[str] = mapped_column(Uuid, ForeignKey("rules.id", ondelete="CASCADE"), nullable=False)
    rule_b_id: Mapped[str] = mapped_column(Uuid, ForeignKey("rules.id", ondelete="CASCADE"), nullable=False)
    package_a_id: Mapped[str] = mapped_column(Uuid, ForeignKey("rule_packages.id", ondelete="CASCADE"), nullable=False)
    package_b_id: Mapped[str] = mapped_column(Uuid, ForeignKey("rule_packages.id", ondelete="CASCADE"), nullable=False)
