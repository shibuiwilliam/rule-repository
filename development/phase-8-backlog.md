...
|---|---|---|---|---|
| A1 | ADR `0004-contract-clause-engine.md` | S | | PENDING |
| A2 | `adapters/contract_parser.py` — DOCX/PDF clause extraction with structure preservation | M | | PENDING |
| A3 | `adapters/contract_compare.py` — semantic clause-level diffing | M | | PENDING |
| A4 | `services/evaluation/clause_aggregator.py` — clause-by-clause verdict → contract-level verdict | M | | PENDING |
| A5 | Upgrade `ClauseSetSubject` with full prompt templates under `subjects/prompts/clause_set/` | M | | PENDING |
| A6 | Evaluation modes: self-conformance, cross-contract conflict, regulatory compliance, risk scoring | L | | PENDING |
| A7 | `POST /api/v1/evaluate/contract` endpoint | S | | PENDING |
| A8 | Unit tests (mocked LLM) + integration tests | M | | PENDING |
...
| # | Work Item | Size | PR | Status |
|---|---|---|---|---|
| B1 | ADR `0005-event-engine-temporal-modes.md` | S | | PENDING |
| B2 | `EventWindow` and `SequenceContext` domain types | S | | PENDING |
| B3 | Sequence-aware evaluation mode — monthly accumulations in rule context | M | | PENDING |
| B4 | Calendar-aware evaluation mode — annual ceilings, 36-Agreement thresholds | M | | PENDING |
| B5 | `POST /api/v1/evaluate/event` update to accept `mode` parameter (single/sequence/calendar) | S | | PENDING |
| B6 | HR system adapter stubs: `adapters/hr_systems/{workday,smarthr,freee_hr}.py` | M | | PENDING |
| B7 | Unit tests per mode + integration test with HR attendance pack | M | | PENDING |
...
| # | Work Item | Size | PR | Status |
|---|---|---|---|---|
| C1 | `DocumentSource` and `IncrementalSource` protocols in `services/discovery/` | S | | PENDING |
| C2 | Upgrade `regulation_pdf.py` to full `DocumentSource` implementation | M | | PENDING |
| C3 | Upgrade `contract_docx.py` to full implementation (python-docx clause extraction) | M | | PENDING |
| C4 | `contract_corpus.py` — cluster historical contracts, extract de facto standard clauses | L | | PENDING |
| C5 | `regulation_feed.py` — e-Gov API / FSA notices with `derives_from` Neo4j linkage | L | | PENDING |
| C6 | SharePoint connector (`services/discovery/connectors/sharepoint.py`) | M | | PENDING |
| C7 | Google Drive connector (`services/discovery/connectors/google_drive.py`) | M | | PENDING |
...
| C10 | Unit tests with mocked external services | M | | PENDING |
...
| # | Work Item | Size | PR | Status |
|---|---|---|---|---|
| D1 | Route groups: `app/(legal)/`, `app/(hr)/`, `app/(finance)/`, `app/(marketing)/` with shared layouts | M | | PENDING |
| D2 | `/contracts/review/[id]` — clause-by-clause verdict view with standard-clause overlay | L | | PENDING |
| D3 | `/events/[id]` — event submission view with applicable rules and verdict | M | | PENDING |
...
- [ ] All four streams pass their Definitions of Done
...
- [ ] CLAUDE.md updated for any new services, dependencies, or architectural decisions
- [ ] IMPROVEMENT.md gaps marked as resolved
- [ ] Eval harness test sets for `CLAUSE_SET` and `EVENT` sequence/calendar modes
- [ ] Demo script at `development/demos/phase-8-demo.md`
