...
**Source code**: `apps/server/src/rulerepo_server/workers/`

The Rule Repository uses **arq** (async Redis queue) for background job processing. The worker runs as a separate container alongside the main server.

### Infrastructure

| Component | Details |
|---|---|
| Job queue | Redis 7-alpine (port 6379) |
| Worker | arq process running `WorkerSettings` from `workers/settings.py` |
| Docker service | `arq-worker` (depends on `redis` and `server`) |
| Configuration | `REDIS_URL` env var (default: `redis://redis:6379/0`) |
...
1. Queries all rules with status `APPROVED` or `EFFECTIVE`.
2. For each rule, computes the health score using the deterministic scorer (`intelligence/health_scorer.py`).
3. Fetches per-rule analytics from the audit log (90-day window).
4. Upserts a `RuleHealthScoreModel` row (deletes old, inserts new).
5. Creates `AlertModel` entries for unhealthy rules:
   - `health_decline` alert when overall score drops below 40.
   - `dormant_rule` alert when activity score is 0 (no evaluations in 90 days).

**`generate_recommendations_task`**:
1. Queries all rules with status `APPROVED` or `EFFECTIVE`.
2. For each rule, computes health score and fetches 90-day analytics.
...
2. Aggregates corrections by `status` (e.g., `pending`, `approved`, `dismissed`).
3. Logs the aggregated statistics via structlog.

### Alert Generation

Alerts are generated as side effects of background worker cron jobs, not through direct API calls:

| Alert Type | Generated By | Trigger Condition |
|---|---|---|
| `health_decline` | `compute_health_scores` | Overall health score < 40 |
| `dormant_rule` | `compute_health_scores` | Activity score == 0 (no evaluations in 90 days) |
| `high_deny_rate` | `generate_recommendations_task` | Deny rate > 50% with >= 10 evaluations |

Alerts are stored as `AlertModel` rows in PostgreSQL and managed through the Alerts API (`/api/v1/alerts`).

### Docker Configuration

The `docker-compose.yml` defines two services for the worker infrastructure:
...
arq-worker:
  ...
  command: ["python", "-m", "arq", "rulerepo_server.workers.settings.WorkerSettings"]
```

The arq-worker uses the same Docker image as the server (built from `infra/docker/server.Dockerfile`) but runs the arq worker process instead of the FastAPI server. The `WorkerSettings.functions` list is empty (no on-demand tasks registered yet). The `workers/tasks.py` module is reserved for future on-demand tasks that are enqueued explicitly from API handlers.
